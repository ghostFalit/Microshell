...
int read_command(char *buffer)
{
    ...
    while (1)
    {
        ...
        else if (!iscntrl(c) && len < MAX_CMD_LEN - 1)
        {
            if (pos == len)
            {
                buffer[pos] = c;
...
void builtin_help()
{
    ...
    printf("2) Dodatkowe bajery: login, kolory, CTRL+C, cudzysłów, clear, strzałki, historia, CTRL+L, CTRL+D\n");
...
int main()
{
    ...
    {
        fprintf(stderr, "Microshell musi być w terminalu.\n");
        return 1;
    }
